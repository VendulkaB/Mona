...
function handleKeyDown(event) {
   ...
   switch(event.key) {
       ...
       case 'ArrowLeft':
           ...
           if (leftCell && !leftCell.disabled) leftCell.focus();
           break;
       case 'ArrowRight':
           const rightCell = document.querySelector(
               `input[data-row="${currentRow}"][data-col="${currentCol + 1}"]`
           );
           if (rightCell && !rightCell.disabled) rightCell.focus();
           break;
       case 'ArrowUp':
...
function generateClues() {
   ...
   const acrossWords = Object.values(crosswordData.words)
       .filter(word => word.direction === 'across')
       .sort((a, b) => a.number - b.number);
   
   const downWords = Object.values(crosswordData.words)
